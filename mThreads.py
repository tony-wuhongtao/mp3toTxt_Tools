...
#     api = RequestApi(
#         appid="04890617", secret_key="6f0cef37fd82d43b8e2d9bd3af6d7874", upload_file_path=r"./todo/sx.mp3")
#     api.all_api_request()

def process_file(file_path, audio_dir, appid, secret_key, semaphore):
    with semaphore:
        base_filename = os.path.splitext(os.path.basename(file_path))[0]
        print("==>" + base_filename)
        
        new_file_path = os.path.join(audio_dir, base_filename + ".mp3")
        os.rename(file_path, new_file_path)
        
        api = RequestApi(appid=appid, secret_key=secret_key, upload_file_path=new_file_path)
        api.all_api_request()
...
if __name__ == '__main__':
    # 设定APPID和SecretKey
    appid = "04890617"
    secret_key = "6f0cef37fd82d43b8e2d9bd3af6d787s"
    ...
    for filename in os.listdir(audio_dir):
        if filename.endswith(".mp4"):
            ...
            thread = Thread(target=process_file, args=(file_path, audio_dir, appid, secret_key, semaphore))
